...
var FirecrawlSearchProvider = class {
	...
	*/
	...
	async search(request, signal) {
		...
		if (slots.length === 0) throw new WebError("Firecrawl search has no accounts configured; add at least one Firecrawl API key through Plugins > dsh-web-search-firecrawl > 配置, or set the dsh-web-search-firecrawl config", "WEB_PROVIDER_CREDENTIAL_MISSING");
...
const Config = z.object({
	...
	limit: z.number().step(1).min(1).default(5).volatile(),
	strategy: z.union(["round-robin", "least-loaded"]).default("round-robin").volatile(),
...
function readSection(config) {
	return {
		accounts: config.accounts.get()?.map((account) => ({ ...account })),
...
function apply(ctx, config) {
	...
	ctx.inject(["settings"], (settingsCtx) => {
		...
		return (async () => {
			...
			if (migration !== void 0) await settingsCtx.settings.update(FIRECRAWL_SETTINGS_NAMESPACE, migration);
		})();
	});
